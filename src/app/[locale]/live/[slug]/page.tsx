    ...
    <>
      ...
      <article className="mx-auto max-w-[1440px] px-[--spacing-gutter] py-6">
        ...
        <div className="mx-auto mt-8 grid max-w-[1100px] gap-8 lg:grid-cols-[minmax(0,1fr)_20rem]">
          {/* ------------------------------------------------ live updates */}
          <section>
            <div className="mb-4 flex items-center gap-2 border-b-2 border-[--color-body] pb-2">
              <span
              ...
              <span className="ms-auto text-xs text-[--color-faint]">
                {t('newestFirst')}
              </span>
            </div>
            ...
            <ol className="space-y-6">
              {story.liveUpdates.map((update) => (
                <li
                  key={update.id}
                  id={update.id}
                  className="scroll-mt-24 border-s-2 border-[--color-line] ps-4"
                >
                  <div className="flex flex-wrap items-center gap-2">
                    <time
                      dateTime={machineDate(update.time)}
                      className="numeric text-sm font-bold text-[--color-live]"
                    >
                    ...
                    <a
                      href={`#${update.id}`}
                      className="ms-auto text-xs text-[--color-faint] hover:text-[--color-body]"
                  ...
                  {update.body && (
                    <p className="mt-1.5 leading-relaxed text-[--color-muted]">
                      {update.body}
                    </p>
                  )}
                </li>
          ...
          {/* -------------------------------------------------- standing copy */}
          <aside className="lg:border-s lg:border-[--color-line] lg:ps-8">
